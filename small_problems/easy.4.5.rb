# Multiples of 3 and 5
# Write a method that searches for all multiples of 3 or 5 that lie between 
# 1 and some other number, and then computes the sum of those multiples. For 
# instance, if the supplied number is 20, 
# the result should be 98 (3 + 5 + 6 + 9 + 10 + 12 + 15 + 18 + 20).

# You may assume that the number passed in is an integer greater than 1.

# Examples

=begin
input: integer, returns integer (sum of multiples)

rules: integer > 1. 

DS: integer, array

Algo:
def method one parameter n
init empty array
- upto, create array using range iterate, count - 1 % 3 or 5 append to array.
assign values (1..n).to_a iterate using each |num|
array < if num % 3 or if num % 5
end
add together using reduce
=end

def multisum(n)
  multiples_array = []
  (1..n).to_a.each do |num|
    multiples_array << num if num % 3 == 0 || num % 5 == 0
  end
  multiples_array.reduce(:+)
end

p multisum(3) == 3
p multisum(5) == 8
p multisum(10) == 33
p multisum(1000) == 234168